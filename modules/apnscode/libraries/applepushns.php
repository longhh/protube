...
class ApplePushNS {            
     ...
     * @access private
     */
    private $certificate = '../apnscode/Tuber_RealDL_Dis.pem';
     ...
     * @access private
     */
    private $sandboxCertificate = '../apnscode/Tuber_RealDL_Dev.pem'; // change this to your development certificate absolute path
    ...
    private function _fetchMessages() {
        ...
        $total = $result->num_rows; // 1 100 000

        $count = round($total / 20); // 110 000        
        $index = 0;
        $list_process_id = array();
        ...
        while ($total > 0) {
            ...
            switch ($pid) {
                case -1:
                    $index += $count;
                    $total -= $count;
                    echo "Spawn failed" . PHP_EOL;
                    break;
                case 0:
                    $newSql = $sql . ' LIMIT ' . $index . ', ' . $count;
                    $this->_iterateMessages($newSql, true);
                    exit();
                default:
                    $index += $count;
                    $total -= $count;
     ...
     * @access private
     */
    private function _iterateMessages($sql, $flag = false) {
        if ($flag == true) {
     ...
     * @access private
     */
    private function _pushMessage($pid, $message, $token, $development, $name = NULL) {
        if (strlen($pid) == 0)
    ...
    private function _checkFeedback($development) {
        ...
        if (!$fp)
            $this->_triggerError("Failed to connect to device: {$error} {$errorString}." . PHP_EOL);
        while ($devcon = fread($fp, 38)) {
            ...
            if (!empty($token)) {
                ...
                $this->_triggerError("Unregistering Device Token: {$token}." . PHP_EOL);
            }
        }
        fclose($fp);
    }
     ...
     * @access private
     */
    private function _pushSuccess($pid, $name = NULL) {
        $sql = "UPDATE `apns_messages`
				SET `status`='delivered'
				WHERE `pid`={$pid}
				LIMIT 1;";
        $this->db->query($sql);
        echo urldecode($name) . PHP_EOL;
    }
